  ...
  const enteredColumns = [
    ...
    },
    {
      ...
      render: (value: string | null) =>
        value ? dayjs(value).format("MMM D, YYYY HH:mm") : "Still Parked",
    },
  ];

  const outgoingTableData =
    outgoingCarsQuery.data?.data?.map((entry: CarEntry) => ({
      ...entry,
    })) || [];

  const enteredTableData =
    enteredCarsQuery.data?.data?.map((entry: CarEntry) => ({
      ...entry,
    })) || [];

  return (
    <div className="px-6 pt-2">
      <h2 className="text-2xl font-bold mb-4 text-center">Car report</h2>
      <div className="">
            ...
            <Popover>
              <PopoverTrigger asChild>
                <Button
                  variant="outline"
                  className={cn(
                    "w-[280px] justify-start text-left font-normal bg-white shadow-none",
        ...
        <>
          ...
          {outgoingCarsQuery.isLoading ? (
            ...
            <p className="text-red-600">
              Error: {outgoingCarsQuery.error.message}
            </p>
          ) : (
            <Table
              paginate
          ...
          {enteredCarsQuery.isLoading ? (
            ...
            <p className="text-red-600">
              Error: {enteredCarsQuery.error.message}
            </p>
          ) : (
            <Table
              paginate
